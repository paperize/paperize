...
v-layout(column)
  v-flex
    ...
    v-dialog(v-model="showNewLayerDialog" max-width="500" lazy)
      v-card
        ...
        v-card-actions
          v-btn(text @click="showNewLayerDialog = false") Cancel
          v-btn(text @click="createLayer('text')") Text
          v-btn(text @click="createLayer('image')") Image
          v-btn(text @click="createLayer('shape')") Shape
  ...
  v-card(v-if="noLayers")
    ...
    v-card-text
      v-list
        v-list-item
          v-list-item-content To get started:
          v-list-item-action
        ...
        v-list-item
          v-list-item-content
            v-select.template-selector(filled label="Select a Template" v-model="componentIdToCopy" :items="copyableComponents" item-value="id" item-text="title")
          v-list-item-action
