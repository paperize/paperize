...
const
  ...
  getIndex = function(folderIds, options={}) {
    ...
    // Make the request
    // TODO: audit every getClient like this
    return new Promise((resolve) => {
      getClient((client) => {
  ...
  findFolders = function(folderName) {
    return getClient(async client => {
      const folderResponse = await client.drive.files.list({
        // Query for by the given name
        q: `mimeType = 'application/vnd.google-apps.folder' and name = '${ folderName }' and trashed = false`,
      })

      return map(folderResponse.result.files, "id")
    })
  },
  ...
  findFile = function(filename, mimeType, foldersToSearch) {
    ...
    return Bluebird.each(foldersToSearch, (folderId) => {
  ...
  createFile = function(name, mimeType, folder, contents, options={}) {
    return new Promise((resolve, reject) => {
      if(!name || !mimeType || !folder || !contents) {
        reject(new Error("drive.createFile() requires name, mimeType, parent folder, and contents arguments!"))
        return
      }
