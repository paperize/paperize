...
async function _renderTextLayer (game, component, template, item, page, layer, options) {
  ...
  _embedInlineFonts(game, component, template, item, page, layer, elements, options)
  ...
  // // break projected text into lines
  // // read each line w/ whitespace

  // let textSize = layer.textSize + 1
  ...
  // let lines = projectedText.split(/\r?\n/);
...
const _InlineSpace = {
  type: _INLINE_SPACE,
  // mirror properties from text layer (undefined defaults to layer)
  textFontName : undefined,
...
async function _embedInlineFonts(game, component, template, item, page, layer, elements, options) {
  const 
    defaultFontFamily  = layer.textFontName ,
    defaultFontVariant = layer.textFontStyle;

  // TODO: extract this function
  async function _embedInlineFont(page, fontFamily, url) {
    // TODO: find a better way to query for existing fonts
    if( find(page.defs().children(), child => {
      return child.node.innerHTML.startsWith(`@font-face{font-family:${fontFamily}`)
    }) !== undefined) return

    // load font as base64
    const font = await new Promise(async (resolve, reject) => {
      const reader = new FileReader()
    ...
    page.defs().fontface(fontFamily, `url(${font})`)
  }

  // embed default font
  const
    googleFont        = find(store.getters.googleFontsRaw, { family: defaultFontFamily }),
    googleFontVariant = googleFont       &&       googleFont.variants[defaultFontVariant];
  if(googleFontVariant) await _embedInlineFont(page, defaultFontFamily, googleFontVariant)  
  ...
  await Promise.all(map(elements, async element => {
    ...
    const 
      fontFamily  = element.textFontName  || defaultFontFamily ,
      fontVariant = element.textFontStyle || defaultFontVariant;
      // embed google font
      const
        googleFont        = find(store.getters.googleFontsRaw, { family: fontFamily }),
        googleFontVariant = googleFont       &&       googleFont.variants[fontVariant];
      if(googleFontVariant) await _embedInlineFont(page, fontFamily, googleFontVariant)
  }))
}
...
function _layoutInlineElements(game, component, template, item, page, layer, elements, options) {
  ...
  // we just need to compute sizes for elements
  // this is going to be tricky methinks

  for(let size = layer.textSize; size >= 4; size --) {

    // compute the size of every element
  }
