...
export const JPG_FORMAT = 'image/jpeg'
...
export const DEFAULT_RASTER_OUTPUT_FORMAT  = PNG_FORMAT
...
function svgToIMG(svg, options) {
    return new Promise( async (resolve, reject) => {
        ...
        if(Array.isArray(svg)) {
            const imgs = [ ]
            for(let _svg of svg)
                imgs.push(await svgToIMG(_svg, options))
            return resolve(imgs)
        }
        ...
        console.log(options.rasterOutputFormat)
