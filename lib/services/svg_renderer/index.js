...
async function _embedInlineFont (page, font) {
  ...
  const fontBase64 = await new Promise( async (res, rej) => {
    const
      ...
      // make font url scheme-relative
      fontLocation = font.location.replace(/^http(s?):/, "")

    reader.onload  = () => res(reader.result)
    reader.onerror = () => rej(             )
    reader.readAsDataURL(await(await fetch(fontLocation)).blob())
  })

  const fontface = await new FontFace(font.family, `url(${fontBase64})`).load()
...
function _layoutInlineElements(game, component, template, item, page, layer, elements, options) {
  ...
  for(; preferredSize >= 4; preferredSize --) {
    ...
    for(const element of elements) {
      ...
      case _INLINE_SPACE: {
        ...
        if(line.totalWidth + elementWidth >= w) {
          // new line
          linesHeight += lineHeight
          if(linesHeight + lineHeight > h)
            continue shrinkToFit
          ...
          line.lineElements.push( element )
        }
      } break
      case _INLINE_TEXT : {
        ...
        if(line.totalWidth + elementWidth > w) {
          ...
          const previous = line.lineElements.pop()
          if(previous && previous.type !== _INLINE_SPACE)
            line.lineElements.push(previous)
          else {
  ...
  for(const line of lines) {
    let
      tx = x,
      ty = y,
      ws = (w - line.textWidth) / line.spaceWidth

    switch(layer.horizontalAlignment) {
      case   'left': /* do nothing */ break;
      case 'center': tx += (w - line.totalWidth) / 2; break;
      case  'right': tx += (w - line.totalWidth)    ; break;
    ...
    switch(layer.verticalAlignment) {
      ...
      case 'middle': ty += (h - linesHeight) / 2; break;
      case 'bottom': ty += (h - linesHeight)    ; break;
    }

    for(const element of line.lineElements) {
      element.x += tx
      element.y += ty
...
async function _renderImageLayer(game, component, template, item, page, layer, options) {
  ...
  try {
    imagePath = (await imagePromise).src
  } catch {
