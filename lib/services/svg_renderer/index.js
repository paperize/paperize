...
export async function renderItemToSVG(game, component, template, item, options) {
    ...
    return await options.renderContext.svg()
}

export async function renderItemToSVGCanvas(game, component, template, item, options={}) {
    return new Promise(async resolve => {
        const
            svgString = await renderItemToSVG(game, component, template, item, options),
        ...
        svgImg.onload = () => {
            canvas.width = options.rasterImageW || svgImg.width
            canvas.height = options.rasterImageH || svgImg.height
            context2D.drawImage(svgImg, 0, 0)
            resolve(canvas)
        }
    })
}
...
export async function renderItemToPNG(game, component, template, item, options) {
    return (await renderItemToSVGCanvas(game, component, template, item, options)).toDataURL(RASTER_IMAGE_PNG_TYPE)
}

export async function renderItemToJPG(game, component, template, item, options) {
    return (await renderItemToSVGCanvas(game, component, template, item, options)).toDataURL(RASTER_IMAGE_JPG_TYPE)
}
...
function _renderItem (game, component, template, item, options) {
    ...
    itemContext.rect(100,100).fill('#f03').move(100,100)
