...
import { SVG, Color, A } from '@svgdotjs/svg.js'
...
async function _renderTextLayer (game, component, template, item, page, layer, options) {
    ...
    // const text = g.text().move(0, 12)

    // text.build(true )


    //     text.tspan('HELLO WORLD 1').fill('#fff').x(0).dy(16)
    //     text.tspan('hello world 2').fill('#fff').x(0).dy(16)
    //     text.tspan('hello world 3').fill('#fff').x(0).dy(16)
    // text.build(false)
