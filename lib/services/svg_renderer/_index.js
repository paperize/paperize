...
async function _renderItems(options) {
  for(const item = 0; item < options.items.length; item ++) {
    const
      page = Math.floor( item / options.itemsPerPage ),
      row  = Math.floor((item - options.itemsPerPage * page) / options.itemsPerRow      ),
      col  =           ((item - options.itemsPerPage * page) - options.itemsPerRow * row)

      options.itemOffsetX = options.pageMarginL + 
  }

}

async function _renderComponent (options) {
  //TODO: per component print/export settings
  const 
    items    = store.getters.getComponentItems    (options.component),
    template = store.getters.findComponentTemplate(options.component),
    pages    = _preparePages({ ...options, template, items })

  return await _renderItems({ ...options, template, items, pages })
}

async function _renderComponents(options) {
  return (await Promise.all(map(options.components, async component => await _renderComponent({...options, component})))).flat()
}

async function _renderGame(options) {
  const components =  store.getters.findAllPrintableGameComponents(options.game)
  return await _renderComponents({ ...options, components })
}
...
function _preparePage (options) {
  return _preparePages(options)[0]
}

function _preparePages(options) {
  options = { ...DEFAULT_UNIT_OPTIONS, ...DEFAULT_PAGE_OPTIONS, ...options }
  if(!options.template) options.template = store.getters.findComponentTemplate(component)
  if(!options.items   ) options.items    = store.getters.getComponentItems    (component)
  const
    itemW = convert(options.template.size.w, INCHES, PIXELS, options),
    itemH = convert(options.template.size.h, INCHES, PIXELS, options),
  ...
  if(!options.itemsPerPage                     ) options.itemsPerPage = options.items.length
  ...
  const numberOfPages = Math.ceil(options.items.length / options.itemsPerPage)
}
