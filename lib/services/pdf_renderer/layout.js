...
export function doLayout(componentSizes, printSettings) {
  if(printSettings.mode == MODE_AUTO_LAYOUT) {
    return autoLayoutItems(componentSizes, printSettings)
  } else if(printSettings.mode == MODE_COMPONENT_PER_PAGE) {
    return componentPerPageItemLayout(componentSizes, printSettings)
  }
}
...
export function componentPerPageItemLayout(componentSizes) {
  let currentPage = 0
