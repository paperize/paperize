...
import { findProperty, percentOfParent, multiplyDimensions } from '../pdf_renderer/helpers'
  ...
  publicApi = {
    renderItemToGraphics(graphics, game, component, item, template) {
      ...
      return Promise.each(layers, layer => {
        const
          layerDimensions = percentOfParent(store.getters.getLayerDimensions(layer), multiplyDimensions(template.size, 150)),
