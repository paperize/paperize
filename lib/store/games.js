...
const GameModel = {
  ...
  getters: {
    ...
    findAllPrintableGameComponents: (_, __, ___, rootGetters) => game => {
      return rootGetters.findAllComponents(game.componentIds).filter(c => {
        const quantityItems = rootGetters.getComponentItems(c).length
        // If there's no rows for the component or it doesn't have a template, no need to render it
        if(!c.templateId || quantityItems == 0) { return false }
        const template = rootGetters.findTemplate(c.templateId)
        // If there's no layers, no need to render the component
        if (template.layerIds.length == 0) { return null }
        return true
      })
    },
