...
import { SELECT_COMPONENTS, ALL_COMPONENTS } from './print';
...
const GameModel = {
  ...
  create(newGame={}) {
    return {
      ...
      spreadsheetId: null,
      printSettings: {
        componentSelection: ALL_COMPONENTS,
        componentIdsToPrint: []
      },
      componentIds:  [ ],
      // Override with given
  ...
  getters: {
    ...
    findAllPrintableGameComponents: (_, getters, __, rootGetters) => game => {
      ...
      return filter(getters.findAllGameComponents(game), (component) => {
        // ...have been selected to print, or printing all components
        if (game.printSettings !== undefined && game.printSettings.componentSelection === SELECT_COMPONENTS
        && !game.printSettings.componentIdsToPrint.includes(component.id)) {
          return false
        }
