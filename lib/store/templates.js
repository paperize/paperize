import { capitalize, defaults, find, includes, isFinite, map, max, reduce, orderBy } from 'lodash'
...
const TemplateModel = {
  ...
  getters: {
    ...
    projectItemThroughTemplate: (_, getters, __, rootGetters) => (item, template, scalingFactors={}) => {
      const
        scaledTemplateSize = (scalingFactors.size && scalingFactors.size != 1) ?
          scaleDimensions(template.size, scalingFactors.size) :
          template.size,

        layers = map(getters.findAllTemplateLayers(template), (layer) => {
          const
            dimensions = rootGetters.getLayerDimensions(layer),
            scaledDimensions = percentOfParent(dimensions, scaledTemplateSize),
      ...
      return Promise.resolve(projectItem(item, layers, scalingFactors))
        .tap((projectedItem) => projectedItem.size = scaledTemplateSize)
    },
