...
export function generateCrud(model) {
  ...
  actions[copyModelName] = ({ dispatch, commit, getters }, existingModel) => {
    ...
    let modifiedExistingModel = { ...existingModel }
    let newModel = model.create(modifiedExistingModel)

    // Initialize relationships of hasMany. We find the list of Ids and we do deep copy
    let relationshipsToInitializeHasMultiple = filter(model.relationships, (relation) => relation.relation == "hasMany")
    relationshipsToInitializeHasMultiple.forEach(relationship => {
      let relationIds = existingModel[`${relationship.model}Ids`]
      let relationshipFindModelName = camelCase(`find ${relationship.model}`)
      let relationshipCopyModelName = camelCase(`copy ${relationship.model}`)
      return Promise.map(relationIds, (relationId) => {
        let relationModel = getters[relationshipFindModelName](relationId, true)
        return dispatch(relationshipCopyModelName, relationModel)
      }).then((relationObjectIds) => {
        const updateModel = { ...newModel }
        updateModel[`${relationship.model}Ids`] = relationObjectIds
        dispatch(updateModelName, updateModel)
      })
    })

    let relationshipsToInitialize = filter(model.relationships, (relation) => relation.initialize)
    return Promise.map(relationshipsToInitialize, (relationship) => {
      let relationId = existingModel[`${relationship.model}Id`]
      let relationshipFindModelName = camelCase(`find ${relationship.model}`)
      let relationModel = getters[relationshipFindModelName](relationId, true)
      let relationshipCopyModelName = camelCase(`copy ${relationship.model}`)
      return dispatch(relationshipCopyModelName, relationModel)
  ...
  actions[destroyModelName] = ({ getters, rootGetters, dispatch, commit }, modelToDestroy) => {
    ...
    let referencesToNullify = filter(model.relationships, { relation: 'referencedBy', dependent: "nullify" })
