import { difference, every, flatten, includes, map, pick, take, times, without } from 'lodash'
...
const FolderModel = {
  ...
  actions: {
    ...
    refreshFolderIndex({ dispatch, rootGetters }, { folderId, nesting = NESTING_DEPTH }) {
      let
      ...
      return currentPromise.then(() => {
        ...
        // recall all the ids we just found in Drive
        trackedFolderIds = flatten(trackedFolderIds)
        trackedSheetIds = flatten(trackedSheetIds)
        trackedImageIds = flatten(trackedImageIds)

        const
          // recall all the ids we have stored in our index
          allFolderIds = map(rootGetters.allFolders, "id"),
          allSheetIds = map(rootGetters.allSpreadsheets, "id"),
          allImageIds = map(rootGetters.allImages, "id")

        // remove Drive assets from indexed assets to get stale assets
        let staleFolderIds = difference(allFolderIds, trackedFolderIds)
        let staleSheetIds = difference(allSheetIds, trackedSheetIds)
        let staleImageIds = difference(allImageIds, trackedImageIds)
